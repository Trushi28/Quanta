// ============================================================
//  shell/shell.c — Quanta Interactive Shell  v2.5
//
//  Phase 2.5 additions:
//    shutdown               — ACPI power-off
//    edit <file>            — built-in nano-style text editor
//    calc <expr>            — infix arithmetic evaluator
//    grep [-i] <pat> <file> — substring search with highlighting
//    wc <file>              — word / line / byte count
//    uname                  — kernel / arch summary
//    which <cmd>            — look up built-in commands
//    motd                   — re-print welcome message
//
//  Fixed:
//    reboot   — now uses power_reboot() (ACPI + KBC + CF9 + triple-fault)
//    sleep    — display shows exact elapsed time measured via uptime_ms
// ============================================================
#include "shell.h"
...
void shell_register(const char *name, const char *help, shell_cmd_fn fn) {
  ...
  strncpy(cmd_table[cmd_count].help, help, 95);
  cmd_table[cmd_count].help[95] = '\0';
...
static void fmt_uptime_ms(uint64_t ms, char out[24]) {
  ...
  int i = 0;
  out[i++] = '0' + (char)((h / 10) % 10);
  out[i++] = '0' + (char)(h % 10);
  out[i++] = ':';
  out[i++] = '0' + (char)((m % 60) / 10);
  out[i++] = '0' + (char)(m % 60 % 10);
  out[i++] = ':';
  out[i++] = '0' + (char)((s % 60) / 10);
  out[i++] = '0' + (char)(s % 60 % 10);
  out[i] = '\0';
}
...
// Supports: ( ) + - * / % with standard precedence and unary minus.
static const char *s_calc_ptr;
static bool s_calc_err;

static int64_t calc_factor(void);
static int64_t calc_term(void);
static int64_t calc_expr(void);

static void calc_skip(void) {
  while (*s_calc_ptr == ' ' || *s_calc_ptr == '\t')
    s_calc_ptr++;
}
...
static int64_t calc_factor(void) {
  ...
  if (*s_calc_ptr >= '0' && *s_calc_ptr <= '9') {
    int64_t v = 0;
    if (s_calc_ptr[0] == '0' &&
        (s_calc_ptr[1] == 'x' || s_calc_ptr[1] == 'X')) {
      s_calc_ptr += 2;
      while ((*s_calc_ptr >= '0' && *s_calc_ptr <= '9') ||
             (*s_calc_ptr >= 'a' && *s_calc_ptr <= 'f') ||
             (*s_calc_ptr >= 'A' && *s_calc_ptr <= 'F')) {
        v = v * 16;
        char c = *s_calc_ptr++;
        if (c >= '0' && c <= '9')
...
static int64_t calc_term(void) {
  int64_t v = calc_factor();
  calc_skip();
  while (*s_calc_ptr == '*' || *s_calc_ptr == '/' || *s_calc_ptr == '%') {
    char op = *s_calc_ptr++;
    int64_t r = calc_factor();
    if (op == '*')
      v *= r;
    else if (r == 0) {
      s_calc_err = true;
...
static int cmd_ls(int argc, char **argv) {
  ...
  char path[VFS_PATH_MAX];
  resolve_path(path_arg ? path_arg : NULL, path, sizeof(path));
  ...
  while (vfs_readdir(fd, idx++, name) == 0) {
    ...
    if (long_fmt) {
      char modestr[11];
      fmt_mode(st.mode, st.type, modestr);
      char tstr[24];
      fmt_uptime_ms(st.mtime, tstr);
      uint32_t nc = (st.type == VFS_DIR)       ? 0x5599FF
                    : (st.type == VFS_CHARDEV) ? 0xFF9922
                                               : 0xDDDDDD;
      fb_set_color(0x778899, FB_COLOR_BLACK);
      kprintf("  %s %2u %6llu  %s  ", modestr, (unsigned)st.nlinks,
              (unsigned long long)st.size, tstr);
      ...
      if (st.type == VFS_FILE) {
        ...
        if (st.size >= 1024 * 1024)
          kprintf("  %4llu MiB", (unsigned long long)(st.size / (1024 * 1024)));
        else if (st.size >= 1024)
          kprintf("  %4llu KiB", (unsigned long long)(st.size / 1024));
        else
          kprintf("  %4llu B  ", (unsigned long long)st.size);
      }
      kprintf("\n");
    }
  }
...
static int cmd_chmod(int argc, char **argv) {
  if (argc < 3) {
    shell_print("Usage: chmod <octal> <path>\n");
    return 1;
  }
  ...
  fb_set_color(0x44EE88, FB_COLOR_BLACK);
  kprintf("  Changed mode of %s to %04o\n", path, (unsigned)mode);
  fb_set_color(FB_COLOR_WHITE, FB_COLOR_BLACK);
  return 0;
}
...
static int cmd_hexdump(int argc, char **argv) {
  if (argc < 2) {
    shell_print("Usage: hexdump <file>\n");
    return 1;
  }
  char path[VFS_PATH_MAX];
  resolve_path(argv[1], path, sizeof(path));
...
static void top_cb(const task_t *t, void *ud) {
  ...
  static const char *states[] = {"RUNNABLE", "RUNNING ", "SLEEPING", "BLOCKED ",
                                 "ZOMBIE  "};
  static const uint32_t scol[] = {0xFFDD88, 0x44EE88, 0x55AAFF, 0xFF9922,
                                  0xFF4444};
  ...
  fb_set_color(0xAADDFF, FB_COLOR_BLACK);
  if (t->last_cpu != 0xFF)
    kprintf("CPU%-2u ", (unsigned)t->last_cpu);
  else
    kprintf("  -   ");
  fb_set_color(0x778899, FB_COLOR_BLACK);
...
static int cmd_free(int argc, char **argv) {
  ...
  pmm_get_stats(&tp, &fp);
  uint64_t up = tp - fp;
  uint64_t tmib = tp * 4096 / (1024 * 1024), umib = up * 4096 / (1024 * 1024),
           fmib = fp * 4096 / (1024 * 1024);
  uint64_t pct = tp ? (up * 100 / tp) : 0;
  fb_set_color(0x00D4FF, FB_COLOR_BLACK);
  kprintf("\n  Memory\n");
  fb_draw_hline('-', 0x335577, FB_COLOR_BLACK);
  kprintf("  Physical  ");
  draw_bar(up, tp, 36);
  fb_set_color(FB_COLOR_WHITE, FB_COLOR_BLACK);
  kprintf("  %llu/%llu MiB  (%llu%%)\n", (unsigned long long)umib,
          (unsigned long long)tmib, (unsigned long long)pct);
  fb_set_color(0x778899, FB_COLOR_BLACK);
  kprintf("            Used: %llu MiB   Free: %llu MiB\n\n",
          (unsigned long long)umib, (unsigned long long)fmib);
  fb_set_color(0xAADDFF, FB_COLOR_BLACK);
...
static int cmd_uptime(int argc, char **argv) {
  (void)argc;
  (void)argv;
  uint64_t ms = sched_uptime_ms(), s = ms / 1000, m = s / 60, h = m / 60;
  fb_set_color(0x33DDCC, FB_COLOR_BLACK);
  kprintf("  %lluh %llum %llus", (unsigned long long)h,
          (unsigned long long)(m % 60), (unsigned long long)(s % 60));
  fb_set_color(0x778899, FB_COLOR_BLACK);
...
#define CROW(l, v, c)                                                          \
  fb_set_color(0xAADDFF, FB_COLOR_BLACK);                                      \
  kprintf("  %-10s", l);                                                       \
  fb_set_color(c, FB_COLOR_BLACK);                                             \
  kprintf("%s\n", v);
  char ncbuf[8];
  ncbuf[0] = '0' + (char)(ncpus % 10);
  ncbuf[1] = '\0';
  CROW("Vendor", vendor, FB_COLOR_WHITE);
  CROW("Brand", brand, FB_COLOR_WHITE);
  CROW("CPUs", ncbuf, 0x44EE88);
  CROW("x2APIC", cpu_has_x2apic() ? "yes" : "no",
       cpu_has_x2apic() ? 0x44EE88 : 0xFF9922);
  CROW("NX bit", cpu_has_nx() ? "yes" : "no",
       cpu_has_nx() ? 0x44EE88 : 0xFF4444);
  CROW("TSC", cpu_has_tsc() ? "yes" : "no",
       cpu_has_tsc() ? 0x44EE88 : 0xFF9922);
...
static int cmd_uname(int argc, char **argv) {
  ...
  uint32_t ncpus = smp ? (uint32_t)smp->cpu_count : 1;
  fb_set_color(0x33DDCC, FB_COLOR_BLACK);
  kprintf("  Quanta OS  v%s (%s)  %s  %u CPU(s)  SMP\n", QUANTA_VERSION,
          QUANTA_CODENAME, QUANTA_ARCH, ncpus);
  fb_set_color(FB_COLOR_WHITE, FB_COLOR_BLACK);
  return 0;
}
...
static int cmd_sleep(int argc, char **argv) {
  ...
  if (ms == 0) {
    shell_print("  (sleep 0 — nothing to do)\n");
    return 0;
  }
  fb_set_color(0x778899, FB_COLOR_BLACK);
  ...
  fb_set_color(0x44EE88, FB_COLOR_BLACK);
  kprintf("  Awake. (requested=%llums, actual=%llums)\n",
          (unsigned long long)ms, (unsigned long long)elapsed);
  fb_set_color(FB_COLOR_WHITE, FB_COLOR_BLACK);
  return 0;
}
...
static int cmd_kv(int argc, char **argv) {
  ...
  if (strcmp(argv[1], "set") == 0) {
    if (argc < 4) {
      shell_print("Usage: kv set <key> <value>\n");
      return 1;
    }
    if (kv_set(argv[2], argv[3]) < 0) {
      fb_set_color(FB_COLOR_RED, FB_COLOR_BLACK);
      kprintf("  kv: set failed\n");
      fb_set_color(FB_COLOR_WHITE, FB_COLOR_BLACK);
      return 1;
    }
    fb_set_color(0x44EE88, FB_COLOR_BLACK);
  ...
  shell_print("  kv: unknown sub '%s'\n  Usage: kv [list|set <k> <v>|get "
              "<k>|del <k>]\n",
              argv[1]);
  return 1;
}
...
// ── calc — infix expression evaluator ────────────────────────────────────
static int cmd_calc(int argc, char **argv) {
  ...
  char expr[SHELL_LINE_MAX] = {0};
  for (int i = 1; i < argc; i++) {
    strcat_safe(expr, argv[i], sizeof(expr));
    if (i < argc - 1)
      strcat_safe(expr, " ", sizeof(expr));
  }

  s_calc_ptr = expr;
  s_calc_err = false;
  int64_t result = calc_expr();
  // Skip trailing whitespace and check for unexpected input
  while (*s_calc_ptr == ' ' || *s_calc_ptr == '\t')
    s_calc_ptr++;

  if (s_calc_err || *s_calc_ptr != '\0') {
    fb_set_color(FB_COLOR_RED, FB_COLOR_BLACK);
    kprintf("  calc: invalid expression\n");
    fb_set_color(FB_COLOR_WHITE, FB_COLOR_BLACK);
    return 1;
  }
...
static int cmd_grep(int argc, char **argv) {
  if (argc < 3) {
    shell_print("Usage: grep [-i] <pattern> <file>\n");
    return 1;
  }
  ...
  char path[VFS_PATH_MAX];
  resolve_path(farg, path, sizeof(path));
  int fd = vfs_open(path, O_RDONLY);
  if (fd < 0) {
    fb_set_color(FB_COLOR_RED, FB_COLOR_BLACK);
    kprintf("  grep: '%s' not found\n", farg);
    fb_set_color(FB_COLOR_WHITE, FB_COLOR_BLACK);
    return 1;
  }

// Helper: case-insensitive character compare
#define CI(a) (icase && (a) >= 'A' && (a) <= 'Z' ? (char)((a) + 32) : (a))
  ...
  while ((n = vfs_read(fd, buf, sizeof(buf))) > 0) {
    for (ssize_t i = 0; i < n; i++) {
      ...
      if (c == '\n' || c == '\0') {
        ...
        linenum++;
        // Substring search
        size_t llen = (size_t)lpos;
        for (size_t j = 0; j + plen <= llen; j++) {
          bool match = true;
          for (size_t k = 0; k < plen; k++) {
            if (CI(line[j + k]) != CI(pat[k])) {
              match = false;
              break;
            }
          }
          if (match) {
            ...
            for (size_t x = 0; x < llen; x++) {
              if (x == j)
                fb_set_color(0xFFFF55, FB_COLOR_BLACK);
              if (x == j + plen)
                fb_set_color(FB_COLOR_WHITE, FB_COLOR_BLACK);
              kprintf("%c", line[x]);
            }
...
static int cmd_wc(int argc, char **argv) {
  ...
  vfs_close(fd);
  fb_set_color(0xAADDFF, FB_COLOR_BLACK);
  kprintf("  %6llu lines  %6llu words  %6llu bytes  %s\n",
...
static int cmd_which(int argc, char **argv) {
  if (argc < 2) {
    shell_print("Usage: which <command>\n");
    return 1;
  }
  for (int i = 0; i < cmd_count; i++) {
    if (strcmp(cmd_table[i].name, argv[1]) == 0) {
      fb_set_color(0x33DDCC, FB_COLOR_BLACK);
      kprintf("  %s: built-in shell command\n", argv[1]);
      fb_set_color(FB_COLOR_WHITE, FB_COLOR_BLACK);
      return 0;
    }
  }
...
// ── motd — re-print welcome message ──────────────────────────────────────
static int cmd_motd(int argc, char **argv) {
  (void)argc;
  (void)argv;
  int fd = vfs_open("/tmp/welcome.txt", O_RDONLY);
  if (fd < 0) {
    shell_print("  (no MOTD found at /tmp/welcome.txt)\n");
    return 1;
  }
  fb_set_color(0x33DDCC, FB_COLOR_BLACK);
  char buf[256];
...
// ── reboot — FIXED: uses power_reboot() ──────────────────────────────────
...
static int cmd_shutdown(int argc, char **argv) {
  ...
  (void)argv;
  fb_set_color(0xFF9922, FB_COLOR_BLACK);
  kprintf("  Shutting down Quanta OS...\n");
  fb_set_color(0x778899, FB_COLOR_BLACK);
...
static void register_builtins(void) {
  // System
  shell_register_cat("version", "Show Quanta banner and version", "System",
                     cmd_version);
  shell_register_cat("uname", "OS / arch summary", "System", cmd_uname);
  shell_register_cat("cpuinfo", "CPU features and topology", "System",
                     cmd_cpuinfo);
  ...
  shell_register_cat("hexdump", "Hex+ASCII dump (first 512 B)", "Files",
                     cmd_hexdump);
  shell_register_cat("cd", "Change directory", "Files", cmd_cd);
  shell_register_cat("pwd", "Print working directory", "Files", cmd_pwd);
  shell_register_cat("grep", "Search file for pattern  [-i=ignore case]",
                     "Files", cmd_grep);
  shell_register_cat("wc", "Count lines / words / bytes", "Files", cmd_wc);
  shell_register_cat("edit", "Open file in built-in text editor", "Files",
                     cmd_edit);
  // Shell
  ...
  shell_register_cat("which", "Check if a command exists", "Shell", cmd_which);
  shell_register_cat("calc", "Evaluate arithmetic expression", "Shell",
                     cmd_calc);
  shell_register_cat("motd", "Show welcome / message of the day", "Shell",
                     cmd_motd);
...
void shell_run(void *arg) {
  ...
  // Auto-display welcome file if it exists
  cmd_motd(0, NULL);
  kprintf("\n");
  ...
  for (;;) {
    ...
    while (!done) {
      ...
      case '\033': {
        ...
        if (s1 == '[') {
          if (s2 == 'A' && hist_pos > 0) {
            ...
            line_redraw();
          } else if (s2 == 'C' && cursor < input_len) {
            cursor++;
            kprintf("\033[C");
          } else if (s2 == 'D' && cursor > 0) {
            cursor--;
            kprintf("\033[D");
          }
          // Swallow 5~ 6~ 3~ sequences at the prompt (not used here)
          else if (s2 == '5' || s2 == '6' || s2 == '3' || s2 == '2')
            kbd_getchar_noblock();
        }
        break;
      }
      case '\t': {
        ...
        for (int i = 0; i < cmd_count; i++)
          if (strncmp(cmd_table[i].name, input_buf, (size_t)input_len) == 0) {
            matches++;
