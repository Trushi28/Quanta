...
void ap_entry(struct limine_smp_info *info) {
    ...
    // Reload our GDT and IDT (they are in kernel memory, already mapped)
    gdt_init();
    idt_reload();

    // Initialise this AP's local APIC
    apic_init();

    // Set up per-CPU GS base
    cpu_local_setup(cpu_id, info->lapic_id);

    // Arm the APIC timer for preemptive scheduling (1 ms tick)
    apic_timer_init(1);
...
void smp_init(void) {
    ...
    kprintf("[SMP] %llu CPU(s) detected  BSP-LAPIC=%u  %s\n",
            (unsigned long long)smp->cpu_count,
            smp->bsp_lapic_id,
            x2apic ? "x2APIC" : "xAPIC");
    ...
    for (uint64_t i = 0; i < smp->cpu_count; i++) {
        ...
        if (ci->lapic_id == smp->bsp_lapic_id) continue;

        uint32_t cpu_id = (uint32_t)(__atomic_load_n(&g_cpu_count, __ATOMIC_SEQ_CST));
        if (cpu_id >= MAX_CPUS) break;

        // Pass the logical CPU ID to the AP via extra_argument
        ci->extra_argument = cpu_id;

        // Atomic write to goto_address wakes the AP (Limine parks it in a
        // loop watching this field).
    ...
    for (uint64_t spin = 0; spin < 100000000ULL; spin++) {
        if (__atomic_load_n(&g_cpu_count, __ATOMIC_SEQ_CST) >= expected) break;
