// ============================================================
//  kmain.c — Quanta OS Kernel Entry Point  v2.3
//
//  Boot flow:
//    fb_init() → fb_splash_draw()          (full-screen splash appears)
//    Each init step calls fb_splash_progress(step, 15, label)
...
// Total init steps shown on the splash progress bar
#define SPLASH_TOTAL 15
...
__attribute__((noreturn)) void kmain(void) {
  ...
  // Draw full-screen splash immediately — from this point kprintf goes
  // to serial only; fb_putchar is a no-op while splash_active == 1.
  ...
  fb_splash_progress(12, SPLASH_TOTAL, "VFS  (ramfs + devfs)");
  ...
  // ── 17. Keyboard ──────────────────────────────────────────────────────
  keyboard_init();
  fb_splash_progress(14, SPLASH_TOTAL, "PS/2 keyboard  (IOAPIC IRQ1)");
  ...
  fb_statusbar_set("Quanta OS v" QUANTA_VERSION
                   "  |  type 'help'  |  'ai <topic>' for QAI");
  fb_statusbar_refresh();

  // ── 19. Launch shell task ─────────────────────────────────────────────
